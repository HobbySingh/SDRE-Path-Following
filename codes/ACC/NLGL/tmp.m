...
waypoints = [[0, 0, 0]; [200, 0, 0]; [200, 400, 0];[0,0,0]];
...
while ( dist_wp >= delta && wp <= last_wp)
    ...
    options = odeset('RelTol',1e-2,'AbsTol',1e-4,'OutputFcn',@(t,y,flag) sl_op_fx_lqr_integral_state(t,y,flag,w1_x,w1_y,w1_z,w2_x,w2_y,w2_z,w_spd_ratio,curr_u_xy,curr_u_z,curr_d_xy,curr_d_z,curr_si));
...
% a = 1;
% u_xy_arr_tmp = filter(b,a,u_xy_arr2);
...
plot(time_arr2(:,1),rad2deg(si_xy_arr2(:,1)),'--b','LineWidth',lw);

xlabel('Time (s)') % x-axis label
ylabel('si : |si_{xy}| degree') % y-axis label
title('Heading')
